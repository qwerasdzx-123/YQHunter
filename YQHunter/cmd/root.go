...
使用示例:
  yqhunter -x https://example.com        # XSS 扫描
  yqhunter -s https://example.com        # SSRF 扫描
...
输出格式:
  ...
  yqhunter -a https://example.com -o result.html         # 输出到 HTML 报告
  yqhunter -p https://example.com -o spider.json         # 爬虫结果导出为 JSON
  yqhunter -d https://example.com -o dirs.txt -F txt     # 指定输出格式`,
}
...
var rootRun = func(cmd *cobra.Command, args []string) {
	...
	if !enableXSS && !enableSSRF && !enableCORS && !enableDir && !enableFingerprint && !enableSpider {
	...
	if outputReport != "" {
		if enableAll {
			...
			if enableSpider && spiderResults != nil {
				err := spider.ExportResults(spiderResults, outputFormat, outputReport)
				if err != nil {
			...
			if enableXSS {
				err := exportVulnerabilities(xssVulns, "XSS", outputReport, outputFormat)
				if err != nil {
					fmt.Printf("导出 XSS 扫描结果失败: %v\n", err)
				}
			}

			if enableSSRF {
				err := exportVulnerabilities(ssrfVulns, "SSRF", outputReport, outputFormat)
				if err != nil {
			...
			if enableCORS {
				err := exportVulnerabilities(corsVulns, "CORS", outputReport, outputFormat)
				if err != nil {
			...
			if enableDir {
				err := exportDirResults(targetURL, dirPaths, outputReport, outputFormat)
				if err != nil {
			...
			if enableFingerprint {
				if useYQFinger {
					yqfingerPath, err := yqfinger.GetYQFingerPath()
					if err != nil {
						useBuiltInFingerprint(targetURL, cfg, outputReport, outputFormat)
					} else {
						client, err := yqfinger.NewYQFingerClient(yqfingerPath)
						if err != nil {
							useBuiltInFingerprint(targetURL, cfg, outputReport, outputFormat)
							...
							if err != nil {
								...
								if outputReport != "" {
									err := exportFingerprintResults(targetURL, convertYQFingerResults(results), outputReport, outputFormat)
									if err != nil {
...
func useBuiltInFingerprint(targetURL string, cfg *config.Config, outputReport string, outputFormat string) {
	...
	if err != nil {
		...
		if outputReport != "" {
			err := exportFingerprints(targetURL, basicFingerprints, outputReport, outputFormat)
			if err != nil {
				fmt.Printf("导出指纹识别结果失败: %v\n", err)
			}
		}
		...
		if outputReport != "" {
			err := exportFingerprintResults(targetURL, fingerprints, outputReport, outputFormat)
			if err != nil {
				fmt.Printf("导出指纹识别结果失败: %v\n", err)
			}
		}
...
func init() {
	...
	rootCmd.Flags().BoolVarP(&enableXSS, "xss", "x", false, "启用 XSS 扫描")
	rootCmd.Flags().BoolVarP(&enableSSRF, "ssrf", "s", false, "启用 SSRF 扫描")
	...
	rootCmd.Flags().StringVarP(&outputFormat, "format", "F", "", "输出格式（html, json, csv, txt，默认根据文件扩展名自动判断）")
...
func exportVulnerabilities(vulns []scanner.Vulnerability, scanType, filename, format string) error {
	if format == "" {
		format = detectFormat(filename)
	}
